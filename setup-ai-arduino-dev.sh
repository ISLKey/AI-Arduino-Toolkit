...
# Version 3.3 - Audited Dependencies Edition
# 
# This script installs everything needed for AI-assisted Arduino development
# on a fresh Ubuntu Desktop installation.
#
# New in v3.3:
# - Comprehensive dependency audit of OpenManus codebase
# - Added 3 critical missing packages: aiohttp, daytona, pandas
# - Now includes 54 dependencies verified against actual imports
# - Should eliminate ALL ModuleNotFoundError issues
#
# v3.2:
# - Added structlog
#
# v3.1:
# - Fixed Daytona config field names
#
# Previous v2.0:
...
print_banner() {
    ...
    echo -e "${CYAN}║                ${YELLOW}v3.3 - Audited Dependencies${CYAN}              ║${NC}"
    echo -e "${CYAN}║                                                            ║${NC}"
...
install_openmanus() {
    ...
    "$HOME/.local/bin/uv" pip install \
        ...
        huggingface-hub setuptools structlog \
        aiohttp daytona pandas
...
configure_openmanus() {
    ...
    # Add Daytona config section (optional feature, but required by config validation)
    # This prevents "Field required" validation error for daytona_api_key
    # Note: Field names must match DaytonaSettings class: daytona_api_key, daytona_server_url
    echo "" >> config/config.toml
    echo "[daytona]" >> config/config.toml
    echo "daytona_api_key = \"not-needed\"" >> config/config.toml
    echo "daytona_server_url = \"https://app.daytona.io/api\"" >> config/config.toml
